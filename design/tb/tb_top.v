...
//   0x0000_1100 : INBUF0  TC0  16B data,  0B pad, IEEE 802.3
//   0x0000_1200 : INBUF1  TC1  48B data, 16B pad, CRC-32C
//   0x0000_1300 : INBUF2  TC2  96B data, 32B pad, IEEE 802.3
//   0x0000_1500 : INBUF3  TC3  48B data,  8B pad, CRC-32C (ciphertext corrupted)
//   0x0000_1600 : OUTBUF0 TC0 out 16B
//   0x0000_1640 : OUTBUF1 TC1 out 48B
//   0x0000_1700 : OUTBUF2 TC2 out 96B
//   0x0000_1780 : OUTBUF3 TC3 out 48B (CRC err — still written)
//
// Test sequence:
//   TC0 : 1-block  decrypt, IEEE 802.3,  interrupt=1 → PAUSE; verify 16B output
//   TC1 : 3-block  decrypt, CRC-32C,    interrupt=1 → PAUSE; verify 48B output, padding
//   TC2 : 6-block  decrypt, IEEE 802.3,  interrupt=1 → PAUSE; verify 96B output, padding
//   TC3 : 3-block  decrypt, CRC-32C,    last=1      → STOP;  verify CRC_ERR (ring wrap!)
//
//   TC4 : BUS ERROR injection — fake_mem returns SLVERR on descriptor fetch
//         Verify STATUS.BUS_ERROR=1, IRQ_BUS_ERROR, state=STOP
//
//   TC5 : INVALID DESCRIPTOR — descriptor starts with valid=0;
//         after INTERVAL cycles TB sets valid=1 via backdoor write;
//         verify descriptor processes correctly
//
//   TC6 : IMMEDIATE STOP — start engine then issue CTRL_IMM_STOP a few cycles
//         later; verify FSM reaches STOP without hanging
//
// Expected plaintext: counter pattern pt[i] = i & 0xFF (generated by gen_mem.c)
//
// Simulator  : NCVerilog (Xcelium)
// Dump format: FSDB (Novas/Verdi) via $fsdbDumpfile / $fsdbDumpvars
//              Falls back to VCD if `NOFSDB is defined
// =============================================================================
