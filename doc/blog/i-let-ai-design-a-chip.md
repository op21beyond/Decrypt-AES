...
이번 작업이 3일쯤 걸린 이유도 AI의 한계만은 아니었다. 처음부터 할 일의 범위를 고정하지 않은 채 진행하면서 사양서를 쓰다 RTL 설계를 붙였고, 뒤늦게 상용 시뮬레이터 라이선스 제약을 떠올려 Verilator 검증을 추가했으며, 코딩이 거의 끝난 뒤에야 다른 AI에게 교차검증을 맡겼다. 책과 블로그로 남기겠다는 결정도 마지막에 내려서, 프롬프트와 작업 순서는 대부분 남아 있지 않은 상태에서 git history로 과정을 복원해야 했다.

반대로 말하면 이런 흐름은 더 짧아질 수 있다. 처음부터 Todo list, 협업 AI의 역할과 교차검증 시점, 단계별 베스트 프롬프트를 레시피처럼 정해 두었다면 이번 과제는 반나절 안에도 끝났을 가능성이 크다. Claude의 dispatcher나 온디바이스 AI 같은 도구는 이런 구조를 더 잘게 이어 붙이게 해 줄 수 있지만, 본질은 특정 도구의 이름보다 협업 방식에 있다. 사양을 먼저 세우고, 구현과 검증을 함께 전개하고, 리뷰와 기록까지 작업 흐름 안에 넣어두면 AI는 하드웨어 개발에서도 충분히 실전적인 동료가 될 수 있다.
